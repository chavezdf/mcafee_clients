...
REM		TITLE: Check-Versions-64bit 
REM		PURPOSE: Check the installed versions of McAfee Clients on 64-bit system.
REM		VERSION: 1.0
REM	-------------------------------------------------------------------------------
...
REG QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Network Associates\TVD\Shared Components\Framework" /v Version > NUL: 2>&1 || GOTO NOEPO64
REM GET EPO VERSION
SET EPOVERQUERY=REG QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Network Associates\TVD\Shared Components\Framework" /v Version
REM PARSE REG QUERY
FOR /F "usebackq tokens=1,2,3" %%a IN (`%EPOVERQUERY% ^| findstr "Version"`) DO SET EPOVER=%%c

REM GET EPO INSTALL DIRECTORY
SET EPOPATHQUERY=REG QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Network Associates\TVD\Shared Components\Framework" /v "Installed Path"
REM PARSE REG QUERY
...
REG QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\McAfee\DesktopProtection" /v szProductVer > NUL: 2>&1 || GOTO NOVSE64
REM GET VSE VERSION
SET VSEVERQUERY=REG QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\McAfee\DesktopProtection" /v szProductVer
REM PARSE REG QUERY
FOR /F "usebackq tokens=1,2,3" %%a IN (`%VSEVERQUERY% ^| findstr "szProductVer"`) DO SET VSEVER=%%c

REM GET VSE INSTALL DIRECTORY
SET VSEPATHQUERY=REG QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\McAfee\DesktopProtection" /v szInstallDir
REM PARSE REG QUERY
